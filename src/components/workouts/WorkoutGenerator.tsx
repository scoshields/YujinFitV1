...
const TIME_LIMITS = [
  { id: '1', duration: 30, label: '30 minutes' },
  { id: '2', duration: 45, label: '45 minutes' },
  { id: '3', duration: 60, label: '60 minutes' },
  { id: '4', duration: 90, label: '90 minutes' }
];
...
export function WorkoutGenerator({ onClose }: { onClose: () => void }) {
    ...
    const loadBodyParts = async () => {
      try {
        const { data, error } = await supabase
          ...
          .select('main_muscle_group')
          .order('main_muscle_group');

        if (error) throw error;
        
        // Get unique muscle groups
        const uniqueGroups = [...new Set(data.map(ex => ex.main_muscle_group))];
        setAvailableBodyParts(uniqueGroups);
      } catch (err) {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const selectedTimeLimit = TIME_LIMITS.find(time => time.id === selectedTime);
      if (!selectedTimeLimit) throw new Error('No time limit selected');

      const selectedGoalData = GOALS.find(goal => goal.id === selectedGoal);
      if (!selectedGoalData) throw new Error('No goal selected');

      // Set rep and set ranges based on goal
      const isStrength = selectedGoalData.id === '1';
      const setRange = isStrength ? { min: 2, max: 6 } : { min: 2, max: 4 };
      const repRange = isStrength ? { min: 3, max: 10 } : { min: 10, max: 15 };
      ...
      for (const bodyPart of selectedBodyParts) {
        ...
        const exercisesForBodyPart = availableExercises
          ...
          .map(ex => ({
            name: ex.name,
            targetSets: Math.floor(Math.random() * (setRange.max - setRange.min + 1)) + setRange.min,
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-black/90 border border-blue-500/10 rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {availableBodyParts.map((muscleGroup) => (
                <button
                  key={muscleGroup}
                  onClick={() => setSelectedBodyParts(prev => 
                    prev.includes(muscleGroup)
                      ? prev.filter(group => group !== muscleGroup) // Remove if already selected
                      : [...prev, muscleGroup] // Add to existing selections
                  )}
                  className={`p-4 rounded-lg border ${
        ...
        {step === 4 && (
          <div className="space-y-4">
            <h3 className="text-xl font-semibold text-white mb-4">Select Time Limit</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="mt-8 flex justify-end">
          ...
          >
            <span>{isLoading ? 'Generating...' : step === 4 ? 'Generate' : 'Next'}</span>
